...
pub struct Config {
    ...
    /// result. However, some data that was "removed" (through filtering, value reassignment, etc.)
    /// may still be present in the various arrow buffers. For example, the data may be in some
    /// values array inside a dictionary array with no keys pointing to it. The transformations are
    /// done this way for best performance. However, if the desire of the transformation was to
    /// redact data for some security purpose, this sanitization pass would be required to actually
    /// ensure the data is removed.
