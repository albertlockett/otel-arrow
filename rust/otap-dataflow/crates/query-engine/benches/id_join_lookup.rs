...
//! benchmark for internal data structure used expr eval joins

use arrow::array::{UInt16Array, UInt32Array};
use criterion::{BenchmarkId, Criterion, criterion_group, criterion_main};
use otel_arrow_dfe_query_engine::pipeline::bench_support::join::{U16IdLookupBenchWrapper, U32IdLookupBenchWrapper};
...
fn bench_id_join_lookup(c: &mut Criterion) {
    ...
    for batch_size in batch_sizes {
        let inputs = (0..batch_size).map(|i| i as u32).collect::<Vec<_>>();
