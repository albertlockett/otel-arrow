...
use super::{ArrayAppend, ArrayBuilder, ArrayBuilderConstructor, ArrayWithType};
...
impl<K, V> DictionaryArrayAppend<K> for PrimitiveDictionaryBuilder<K, V>
where
    K: ArrowDictionaryKeyType,
...
mod test {
    ...
    fn test_primitive_builder() {
        ...
        assert_eq!(
            result.array.as_any().downcast_ref::<UInt8Array>().unwrap(),
            &expected
        );
    }
