...
impl StreamConsumer {
    ...
    // TODO -- should this return a crate::error:Result and we could impl From<ArrowError> ...
...
impl Consumer {
    ...
    pub fn consume_metrics_batches(
        ...
        // TODO, due to what's refactored here, might need to write a test case to cover case where
        // the batches were empty at least ...
        match get_main_payload_type(records)? {
...
fn get_main_payload_type(
    records: &BatchArrowRecords,
) -> error::Result<ArrowPayloadType> {
    ensure!(!records.arrow_payloads.is_empty(), error::EmptyBatchSnafu);

    // by convention, the main record type is the first payload
    // TODO -- confirm tthis?
